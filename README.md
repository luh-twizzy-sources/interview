...
Файл `real_interview.html` запускает смешанную сессию Java backend интервью по локальному банку `interview_question_bank.js`. В конце можно сформировать Markdown/JSON-отчет с вопросами, своими ответами, самооценкой и кратким эталоном, чтобы отдельно проверить качество ответов.
